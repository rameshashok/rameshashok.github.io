    ...
    <section className="relative py-32 px-6 overflow-hidden">
      ...
      <div ref={titleRef} className="relative z-10 mb-20">
        <div className="text-center">
          ...
          <h2 className="text-7xl font-black text-white mb-4 tracking-wider">
            TIMELINE
          </h2>
      ...
      <div ref={timelineRef} className="relative z-10 max-w-5xl mx-auto">
        <div className="absolute left-1/2 transform -translate-x-1/2 w-1 h-full bg-gradient-to-b from-accent via-accent/50 to-transparent"></div>
        
        <div className="space-y-12">
          {allEvents.map((event, i) => (
            <div key={i} className={`flex items-center ${i % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
              <div className={`w-5/12 ${i % 2 === 0 ? 'text-right pr-8' : 'text-left pl-8'}`}>
                <div className="bg-black/20 backdrop-blur-md p-6 rounded-xl border border-accent/20 hover:border-accent/50 transition-all duration-300 hover:shadow-lg hover:shadow-accent/20">
                  <div className="text-accent text-sm font-medium mb-2">{event.date}</div>
                  <h3 className="text-xl font-bold text-white mb-1">{event.title}</h3>
                  <p className="text-slate-400 mb-2">{event.subtitle}</p>
